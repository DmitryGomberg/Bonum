   ...
   return (
      <div className="fixed bottom-0 right-0 p-4 space-y-2 z-50 flex flex-col items-end">
         {notifications.map(({ id, message, type }) => (
            <div
               key={id}
               className={`notification ${type === 'success' ? 'bg-white' : type === 'warning' ? 'bg-orange-500 text-white' : 'bg-red-500 text-white'} border border-brown3 text-brown5 px-4 py-2 rounded-lg shadow-lg flex justify-between items-center`}
            >
               <span>{message}</span>
